...
export function PatientJourneyCompact({ patient }: PatientJourneyCompactProps) {
  ...
  const milestones: Milestone[] = [
    ...
    { id: 'diagnosed', label: 'Diagnosed', sublabel: patient?.tb_diagnosed === 'Y' ? (fmtDate(patient?.tb_diagnosis_date) || 'Confirmed') : patient?.tb_diagnosed === 'N' ? 'Not Confirmed' : 'Pending', icon: <Activity className="w-3 h-3" />, isComplete: Boolean(patient?.tb_diagnosed), isActive: Boolean(patient?.referral_date) && !patient?.tb_diagnosed },
    ...
    { id: 'nikshay', label: 'Nikshay', sublabel: patient?.nikshay_abha_id || 'Pending', icon: <ClipboardList className="w-3 h-3" />, isComplete: Boolean(patient?.nikshay_abha_id), isActive: Boolean(patient?.att_start_date) && !patient?.nikshay_abha_id },
