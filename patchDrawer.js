...
// Remove the accidental duplicate import I just made above (just to be safe)
const importsClean = content.split('import { calculatePatientRisk } from \'@/lib/risk-engine\';');
if (importsClean.length > 2) {
   content = importsClean[0] + "import { calculatePatientRisk } from '@/lib/risk-engine';" + importsClean[1] + importsClean.slice(2).join('');
}


// 2. Global Keybinds & Focus Traps
const oldKeyboardListener = `  // Keyboard shortcuts for power users
  useEffect(() => {
...
const newKeyboardListener = `  // --- ELITE LAYER: Scroll Lock & Focus Accessibility ---
  useEffect(() => {
    if (isOpen) {
...
content = content.replace(oldKeyboardListener, newKeyboardListener);


// 3. Clinical Sync Update
const oldClinicalSync = `      // Triple-sync API call
...
const newClinicalSync = `      // 1. Fire Elite Layer Sync Queue (Auto-falls back to Dexie if offline)
...
content = content.replace(oldClinicalSync, newClinicalSync);


// 4. Close Loop Update
const oldCloseLoop = `      // Triple-sync API call
      const response = await fetch('/api/patient-sync', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
const newCloseLoop = `      // 1. Fire Elite Layer Sync Queue
...
content = content.replace(oldCloseLoop, newCloseLoop);

// 5. Demographics Sync Update
const oldDemoSync = `      // Call the triple-sync API
      const response = await fetch('/api/patient-sync', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
const newDemoSync = `      // 1. Fire Elite Layer Sync Queue
      const syncResult = await queuePatientSync(localPatient.id, 'update', updatesWithIdentifiers);
...
content = content.replace(oldDemoSync, newDemoSync);


// 6. Voice Input Replacement
const oldVoiceInput = `                <label className="block text-sm font-medium text-slate-700 mb-1">
                  Remarks
                </label>
                <textarea
                  {...register('Remarks')}
...
const newVoiceInput = `                <label className="block text-sm font-medium text-slate-700 mb-1">
                  Remarks
                </label>
                <VoiceInput
                  value={watch('Remarks')}
...
content = content.replace(oldVoiceInput, newVoiceInput);


// 7. Morphing Overlay (Top Wrapper)
const oldWrapperTop = `  return (
...
content = content.replace(oldWrapperTop, newWrapperTop);

// 8. Morphing Overlay (Inner Header)
const oldInnerHeader = `        {/* Header with Patient Info */}
...
const newInnerHeader = `        {/* Header with Patient Info */}
...
content = content.replace(oldInnerHeader, newInnerHeader);


// 9. Morphing Overlay (Header End)
const oldInnerHeaderEnd = `          </motion.div>
        </SheetHeader>`;

const newInnerHeaderEnd = `          </motion.div>
        </div>`;

content = content.replace(oldInnerHeaderEnd, newInnerHeaderEnd);


// 10. Morphing Overlay (Bottom Wrapper)
const oldWrapperBottom = `        )}
      </SheetContent>
      </SheetPortal>
    </Sheet>
  );`;

const newWrapperBottom = `        )}
          </motion.div>
        </div>
      )}
    </AnimatePresence>
...
content = content.replace(oldWrapperBottom, newWrapperBottom);

fs.writeFileSync(FILE, content, 'utf8');
console.log('Successfully patched PatientDetailDrawer.tsx via pure JS replacement.');
