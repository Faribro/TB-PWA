...
[2026-01-21] scripts/audit-gujarat-entries.js - Created Gujarat entries audit script to diagnose missing records issue; script performs 5 comprehensive checks: (1) Total count with RLS bypass using service role key, (2) District-level breakdown, (3) Data quality validation (missing names/IDs/districts), (4) Recent entries analysis (last 30 days), (5) Duplicate detection via kobo_uuid and unique_id. Audit revealed 1,302 total Gujarat entries with 1,000 missing unique_id values but no duplicates or missing critical fields. All entries are recent (within 30 days) indicating active data collection. [AQ]

[2026-01-21] scripts/verify-gujarat-monthly.js - Created monthly breakdown verification script with batch fetching to bypass Supabase 1000-row limit; script fetches all 1,302 Gujarat records in 2 batches and groups by month. Diagnosis confirmed: 5 records have invalid screening_date values from 1995-2004 (March 1995: 1, August 1995: 1, December 1995: 1, April 1996: 1, September 2004: 1) which are correctly filtered out by Vertex's date validation, resulting in 1,297 displayed records. Valid monthly distribution: February 2026 (78), March 2026 (306), April 2026 (912), May 2026 (1). Vertex display is working correctly. [AQ]

[2026-01-21] migrations/add_date_corrected_column.sql - Created SQL migration to add date_corrected BOOLEAN column to patients table; column flags records where screening_date was backfilled from submitted_on due to invalid original dates; includes partial index for performance on date_corrected = true queries [AQ]

[2026-01-21] scripts/fix-invalid-dates.js - Created automated script to fix invalid screening_date values (pre-2020) by backfilling from submitted_on column; script identifies 5 Gujarat records with dates from 1995-2004, proposes fixes, and requires --confirm flag for safety; sets date_corrected = true flag for visual highlighting in UI [AQ]

[2026-01-21] components/ScreeningFrequencyTimeline.tsx - Enhanced Neural Timeline to visually highlight date-corrected records with glowing amber indicators; added pulsing animation (2s cycle), amber ring around bars, blinking dot on top, and tooltip showing corrected count; monthlyData now tracks both total count and corrected count per month for accurate visualization [AQ]
...
[2025-01-23] DemographicsCarousel.tsx - Fixed Bug 3: symptoms_10s field showing empty due to data type mismatch. Changed from incorrect select dropdown with options=['Yes','No'] to read-only formatted text display. Added formatSymptoms() helper function that converts Kobo multi-select comma-separated strings (e.g., "Cough of any duration,Night_Sweats,Weight_Loss_2") into readable bullet-separated format ("Cough of any duration • Night Sweats • Weight Loss 2"). Field now displays actual symptom data from database instead of showing empty. Made editable={false} since symptoms are captured at Kobo screening time and should not be edited in demographics tab. [AQ]
...
[2025-01-23] DemographicsCarousel.tsx - Fixed visibility issue for unique_id and address fields. Changed unique_id in TB Screening section to editable=false (was editable=true causing display issues). Added fallback value handling for System Metadata fields (kobo_uuid and serial_number) to show 'Not recorded' when null. Added alternative field name check for serial_number (patient?.unique_id ?? patient?.serial_number) to handle different database schema variations. [AQ]

[2025-01-23] instrumentation.ts, next.config.mjs - CRITICAL FIX: Fixed Google Sheets sync queue not initializing on server startup causing "Queue not initialized - skipping sync" errors. Created instrumentation.ts file to call initSheetsQueue() when Next.js server starts. Enabled experimental.instrumentationHook in next.config.mjs. Queue now initializes automatically with Redis backend (or falls back to in-memory queue if Redis unavailable). This fixes the silent sync failures where patient updates were saving to Supabase but not syncing to Google Sheets. [AQ]

[2025-01-23] lib/sheetsSyncQueue.ts, lib/sheetsSyncHybrid.ts - Improved error handling and fallback behavior for Google Sheets sync. Added try-catch in initSheetsQueue() to gracefully handle Redis connection failures and fall back to in-memory queue. Changed queuePatientSync() to throw errors instead of silently failing, triggering automatic fallback to in-memory queue. Added better logging to show when in-memory fallback is active vs Redis queue. Fixed issue where Redis connection errors (ENOTFOUND) were preventing any sync from happening - now automatically uses in-memory queue when Redis is unavailable. [AQ]

[2025-01-23] lib/redis.ts - Fixed Redis connection to use Upstash credentials. Updated IORedis initialization to check for REDIS_URL first, then automatically construct rediss:// URL from UPSTASH_REDIS_REST_TOKEN and UPSTASH_REDIS_REST_URL environment variables. Added TLS support for rediss:// connections (required by Upstash). Added connection success logging. This fixes the "ENOTFOUND gusc1-merry-mink-40234.upstash.io" errors - the old hardcoded URL was incorrect. Now uses the correct Upstash instance: infinite-snail-94217.upstash.io. [AQ]

[2025-01-23] DemographicsCarousel.tsx - Fixed layout imbalance where all sections except Identity Profile were stacked in right column. Redistributed sections across both columns: Left column now has Identity Profile, Referral & Diagnosis, and HIV/ART. Right column has TB Screening, Nikshay/Registration, and System Metadata. This creates better visual balance and improves space utilization in the demographics drawer. [AQ]
...
[ ]   [ D e m o g r a p h i c s C a r o u s e l . t s x ]   -   R e f a c t o r e d   d e m o g r a p h i c s   s e c t i o n   i n t o   a   p r e m i u m   s i n g l e - p a g e   c l i n i c a l   r e s u m e   l a y o u t .   I m p r o v e d   t y p o g r a p h y ,   e x p l i c i t l y   r e n d e r e d   1 0 S   s y m p t o m   c h e c k l i s t ,   a n d   d i a g n o s e d / f i x e d   a d d r e s s   b u g   b y   c o n c a t e n a t i n g   K o b o   a d d r e s s   p a r t s .   [ A Q ] 
 
 [ 2 0 2 6 - 0 5 - 0 4 ]   [ D e m o g r a p h i c s C a r o u s e l . t s x ]   -   R e f a c t o r e d   d e m o g r a p h i c s   s e c t i o n   i n t o   a   p r e m i u m   s i n g l e - p a g e   c l i n i c a l   r e s u m e   l a y o u t .   I m p r o v e d   t y p o g r a p h y ,   e x p l i c i t l y   r e n d e r e d   1 0 S   s y m p t o m   c h e c k l i s t ,   a n d   d i a g n o s e d / f i x e d   a d d r e s s   b u g   b y   c o n c a t e n a t i n g   K o b o   a d d r e s s   p a r t s .   [ A Q ] 
 
 " [ 2 0 2 5 - 0 1 - 2 1 ]   [ D e m o g r a p h i c s C a r o u s e l . t s x ]   -   A d d e d   l i q u i d   m e t a l   a n i m a t e d   b o r d e r   e f f e c t   w i t h   s h i m m e r   a n i m a t i o n   a n d   h o v e r   g l o w   f o l l o w i n g   p r e m i u m   U I   b e s t   p r a c t i c e s   [ A Q ] "   
 
 " [ 2 0 2 5 - 0 1 - 2 1 ]   [ A n i m a t e d T o g g l e . t s x ,   D e m o g r a p h i c s C a r o u s e l . t s x ,   F i l t e r B a r . t s x ]   -   I m p l e m e n t e d   p r e m i u m   a n i m a t e d   n e o n   t o g g l e   s w i t c h e s   w i t h   F r a m e r   M o t i o n   t h r o u g h o u t   p r o j e c t .   F e a t u r e s :   n e o n   g l o w   e f f e c t s ,   L E D   i n d i c a t o r s ,   s m o o t h   s p r i n g   a n i m a t i o n s ,   n e u m o r p h i c   s h a d o w s ,   a n d   S V G   g r a d i e n t   r i n g s .   R e p l a c e d   a l l   b a s i c   c h e c k b o x e s   w i t h   a n i m a t e d   t o g g l e s .   [ A Q ] "   
 
 [ 2 0 2 6 - 0 5 - 0 4 ]   [ D e m o g r a p h i c s C a r o u s e l . t s x ]   -   R e f a c t o r e d   d e m o g r a p h i c s   t o   a   p r e m i u m   L a T e X - r e s u m e - i n s p i r e d   d e s i g n ,   m a t c h i n g   t h e   u s e r ' s   p r o v i d e d   r e s u m e   a e s t h e t i c .   [ A Q ] 
 
 