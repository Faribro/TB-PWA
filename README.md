...
[2026-05-04] DemographicsCarousel.tsx - Second pass refinement for production-grade clinical UI. Typography: increased field labels from text-[10px] to text-[11px], values from text-[14px] to text-[15px], section titles from text-[10px] to text-[10.5px], empty states from text-[11px] to text-[12px], clinical status labels from text-[9px] to text-[10.5px] for accessibility. Changed symptoms from chip-based to row-based clinical checklist (SymptomRow component) with better scanning, increased contrast between selected/unselected states. Responsive grids: changed all sections to grid-cols-1 sm:grid-cols-2 md:grid-cols-3 for stable narrow drawer widths, address now spans full width (span={3}). Empty states standardized to "Not recorded" across clinical status box. Input styling improved: increased text-[12px] to text-[13px], better padding (px-2.5 py-2), enhanced focus ring (focus:ring-2). Added aria-label to checkbox toggle for accessibility. Increased section gap from gap-4 to gap-5 for better breathing room. High-risk alert text increased from text-[10px] to text-[11px]. All changes maintain edit/save compatibility. [AQ]
...
[2025-01-21] [VERCEL_ENV_SETUP.md, scripts/setup-vercel-env.*] - Added comprehensive Vercel environment variables setup guide with QStash tokens, automated scripts for Windows/Linux, and verification checklist [AQ]
