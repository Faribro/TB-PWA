...
import { ShieldCheck, Terminal, ShieldAlert, Cpu, Activity, Globe, Loader2 } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleDirectEmailSignIn = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
    if (!directEmail.trim() || isSigningIn) return;
    setIsSigningIn(true);
    sounds.glassShatterLogin();
    try {
      ...
      console.error('[login] credentials signin error:', err);
      setIsSigningIn(false);
    }
  }, [directEmail, isSigningIn]);
      ...
      <main className="flex-1 flex flex-col items-center justify-center w-full px-4 pt-10 pb-24 relative z-10">
        ...
        >
          ...
          <CardContent className="p-10 flex flex-col items-center relative z-20 overflow-hidden">
            ...
            {/* Officer Email Sign-in (RBAC Enabled) */}
            <form onSubmit={handleDirectEmailSignIn} className="w-full space-y-4 mb-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="name@allianceindia.org or gmail.com"
                  required
                  className="w-full h-12 px-4 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#004a99] focus:border-transparent text-slate-800 placeholder:text-slate-400 shadow-sm transition-all"
                />
              </div>
              <button
                type="submit"
                disabled={isSigningIn || !directEmail}
                className="w-full h-12 bg-[#004a99] hover:bg-blue-700 text-white font-bold rounded-xl text-sm transition-all flex items-center justify-center gap-2 disabled:opacity-50 shadow-md hover:shadow-lg"
              >
            ...
            <button
              ...
              className="w-full h-11 bg-white flex items-center justify-center gap-3 border border-slate-200 rounded-xl text-slate-600 font-semibold text-xs hover:bg-slate-50 hover:border-slate-300 transition-all group/btn disabled:opacity-60"
                  ...
                  <div className="bg-[#0c130c] p-6 border border-[#33ff99]/20 col-span-2 flex flex-col h-full">
                    ...
                    <div className="flex-shrink-0 mt-3">
                      <motion.button
                        ...
                        disabled={!isUnlocked || isSigningIn}
                        animate={{
                          boxShadow: isUnlocked && !isSigningIn
                            ? ['0 0 0px #33ff99', '0 0 30px #33ff99', '0 0 10px #33ff99']
                            : '0 0 0px transparent'
                        }}
                        transition={{ duration: 0.6, repeat: isUnlocked && !isSigningIn ? 2 : 0 }}
                        className={`w-full py-4 font-black text-sm tracking-[0.3em] transition-all flex items-center justify-center gap-2 ${isUnlocked && !isSigningIn
                      ...
                      >
                        {isSigningIn ? (
                          ...
                          <span>{isUnlocked ? '[ EXECUTE_SYSTEM_BREACH_SIGN_IN ]' : '[ ACCESS_RESTRICTED_ENTER_KEY ]'}</span>
                        )}
                      </motion.button>
                    </div>
                  </div>
